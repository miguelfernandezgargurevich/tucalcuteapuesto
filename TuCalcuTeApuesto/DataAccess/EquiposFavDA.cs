...
{
    public class EquiposFavDA
    {
        dbTuCalcuEntities _db;

        public bool EliminarEquipoFav(EquiposFavoritos obj) {
            try
            {
                _db = new dbTuCalcuEntities();

                var entidad = _db.EquiposFavoritos.Where(m => m.CodEquipo == obj.CodEquipo && m.CodUsuario == obj.CodUsuario).ToList().FirstOrDefault();
                _db.EquiposFavoritos.Remove(entidad);
                _db.SaveChanges();
        ...
        public bool GrabarEquipoFav(EquiposFavoritos obj)
        {
            try
            {
                _db = new dbTuCalcuEntities();
                _db.EquiposFavoritos.Add(obj);
                _db.SaveChanges();
        ...
        public List<EquiposFavModel> ListarEquiposFavoritos(string codUsuario)
        {
            _db = new dbTuCalcuEntities();
            IQueryable<EquiposFavModel> queryResult = from r in _db.EquiposFavoritos
                                                      join tblUsuarios in _db.AspNetUsers on r.CodUsuario equals tblUsuarios.Id
                                                      join tblEquipos in _db.Equipos on r.CodEquipo equals tblEquipos.CodEquipo into gj
                                                      from x in gj.DefaultIfEmpty()
                                                      where tblUsuarios.Id.Equals(codUsuario)
                                                      select new EquiposFavModel
                                                      {
                                                          Text = x.DesEquipo,
                                                          Value = x.CodEquipo,
                                                          Imagen = x.Flag
